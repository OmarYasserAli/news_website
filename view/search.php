...
$search=$_POST['sea'];
	...
	$sql = "SELECT * FROM $tbl_name where topic like '%$search%' order by id DESC LIMIT $start, $limit";  
...
?>
	...
	<?php
			  ...
			  while($row=mysqli_fetch_object($result)){
				  ...
				  ?>
				  <div id="news">
				   <div id="pic"><img src="../image.php?id=<?echo $row->id; ?>" width="230" height="180"/></div>
                <div id="title">
                <br />
                ...
                $dot=strpos($row->topic,'.'); 
                if($dot==0)
                {
                    $dot=320;
                       print  stripcslashes(substr($row->topic,0,$dot));                 
                }
               elseif($dot<50)
                {
                    $dot=320;
